...
/**
 * Validates whether a DateTime instance is before than a defined limit.
 */
#[Attribute(Attribute::TARGET_PROPERTY)]
class MinDateTime extends Validator
{
    /**
     * @param \DateTime $min Minimum acceptable Date/Time.
    ...
    private function checkDateTimeInterval(\DateTime $dt): void
    {
        ...
        if ($dt->getTimestamp() < $this->max->getTimestamp()) {
            $this->throwValidationException("The date/time value of '{$this->propertyName}' is before the minimum acceptable date/time.");
        }
    }
}
