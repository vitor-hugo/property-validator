...
require_once __DIR__ . "/Contracts/IsNegativeContract.php";

#[Group("Validators")]
#[Group("Numbers")]
#[Group("IsNegative")]
#[TestDox("IsNegative Number Validator")]
class IsNegativeTest extends TestCase
{
    private \ValidIsNegativeContract $stub;


    public function setUp(): void
    {
        $this->stub = new \ValidIsNegativeContract();
    }
    ...
    public function testShouldThrowWhenValueIsZero()
    {
        ...
        $this->expectExceptionMessage("Property 'num1' only accepts negative numbers, received 0.");
        $this->stub->num1 = 0;
        $this->stub->validate();
    }


    #[TestDox("Should throw ValidationException when INT is positive")]
    public function testShouldThrowWhenIntIsNegative()
    {
        $this->expectException(ValidationException::class);
        $this->expectExceptionMessage("Property 'num1' only accepts negative numbers, received 13");
        $this->stub->num1 = 13;
        $this->stub->validate();
    }


    #[TestDox("Should throw ValidationException when FLOAT is positive")]
    public function testShouldThrowWhenFloatIsNegative()
    {
        $this->expectException(ValidationException::class);
        $this->expectExceptionMessage("Property 'num2' only accepts negative numbers, received 3.1415.");
    ...
    {
        $this->expectException(ValidationException::class);
        $this->expectExceptionMessage("Why be so positive!?");
        $this->stub->num4 = 999;
        $this->stub->validate();
    }
    ...
    public function testShouldThrowWhenPropertyTypeIsInvalid()
    {
        ...
        $stub = new \InvalidIsNegativeContract();
        $stub->validate();
    }
}
